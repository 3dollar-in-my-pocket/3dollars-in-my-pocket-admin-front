 ...
 */
export const getTimeUntil = (targetDate, currentDate = new Date()) => {
  ...
  if (diffHours < 1) {
    ...
    if (diffMinutes < 10 && remainingSeconds > 0) {
      return `${diffMinutes}분 ${remainingSeconds}초 후`;
    }
  ...
  return `${diffDays}일 ${remainingHours}시간 후`;
};
...
export const getAdStatus = (startDateTime, endDateTime, currentDate = new Date()) => {
  ...
  if (current < start) {
    return {
      status: 'scheduled',
      label: '예정',
      timeText: getTimeUntil(startDateTime, current),
    ...
    return {
      status: 'active',
      label: '진행중',
      timeText: getTimeUntil(endDateTime, current),
