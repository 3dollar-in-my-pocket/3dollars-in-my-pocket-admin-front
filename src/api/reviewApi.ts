...
export default {
   ...
   */
  getStoreReviews: async (storeId: any, cursor: any = null, size = 20) => {
    try {
      const params: any = {
        size,
        includes: 'WRITER'
   ...
   */
  blindStoreReview: async (reviewId: any) => {
    try {
      const response = await axiosInstance({
        method: 'PUT',
   ...
   */
  getUserReviews: async (userId: any, cursor: any = null, size = 20) => {
    try {
      const params: any = {
        size,
        includes: 'STORE',
   ...
   * @param {string} [cursor] - 페이징 커서
   * @param {number} [size=20] - 페이지 사이즈
   * @returns {Promise<Object>} 전체 스토어 리뷰 목록
   */
  getAllStoreReviews: async (cursor: any = null, size = 20) => {
    try {
      const params: any = {
        size,
